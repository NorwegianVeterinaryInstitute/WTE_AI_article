---
title: "SEP H5N5 HA Phylogenetic tree"
...
params:
    ...
    tree: "results/H5N5_HA/iqtree/H5N5_all_SEP_MSA_jalview.fasta.contree"
    save_dir: "results/H5N5_HA"
    env_file: "results/env_files/H5N5_HA_SEP.RData"
    source_code_dir: 'code/R'
execute: 
  echo: false
  cache: false
format: html
...
```{r}
rooted_tree1 <- ape::root(tree, 
                         outgroup = "EPI_ISL_1254",
                         resolve.root = T,
                         edgelabel = T)
```


Dropping isolate that was incomplete and same sequencing more complete
```{r}
rooted_tree1 <- ape::drop.tip(rooted_tree1, "EPI_ISL_12028892")
```

dropping outgroup

```{r}
rooted_tree <-   ape::drop.tip(rooted_tree1, "EPI_ISL_1254")
```
...
<!-- using rooted tree 1 for that because contains also the outgroup -->
```{r}
tree_meta <- 
  tibble::tibble("label" = rooted_tree1$tip.label) %>%
...
This is ok that we have one H5N1 as its the reference/root 
...
explor_plot1 <- 
  ...
  # HERE SHAPE
  ggplot2::scale_shape_manual(values = SEP_years_shape_pal, 
                              name = "Shape Year",
                              na.translate = FALSE) 
explor_plot1
```
...
geom_tippoint(
    ...
    position = ggforce::position_auto(),
  inherit.aes = T) +
  scale_color_manual(values = category_highlight_pal2, 
                              na.translate = FALSE,
...
<!-- the size scale is done to be able to adjust the size of the points - as does not work as usual --> 
...
_Figure Caption:_
H5N5 HA segment. ML phylogenetic tree. The tree is rooted but the root is not shown
...
saveRDS(data_here, here::here(params$save_dir, "H5N5_HA_metadata.rds"))
```

<!-- HERE modified 
append - but I do not save those before so ok
--> 

```{r}
xlsx::write.xlsx(as.data.frame(data_here) %>%
                   dplyr::select(-label), 
                 here::here(params$save_dir, "H5N5_HA_metadata.xlsx"),
                 sheetName = "H5N5_HA_metadata",
                 showNA = FALSE,
                 row.names = FALSE,
                 append = FALSE)
```
