...
explor_plot3d <- 
  ...
  ggplot2::geom_point(aes(x, y, color = category_highlight),
                      data = . %>% filter(isTip), 
                      position = ggforce::position_auto(), size = .5) +
  ggplot2::scale_color_manual(values = category_highlight_pal, 
                              na.translate = FALSE,
                              name = "Highlight") +
...
```{r}
export_meta_treenodes_fun(
  plot_data = explor_plot4$data,
  metadata = all_tree_metadata2,
  col_rep_tips = "tip_metalink",
  file = here::here(params$save_dir, "test_H5N1_all_labels_metadata.xlsx"),
  sheet = "H5N1_all_labels_metadata",
  Isolate_Id, Isolate_Name, year, continent, country, Host_family, Host_latin
)
```
