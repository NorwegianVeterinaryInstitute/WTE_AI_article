---
title: "SEP H5N1 HA all Phylogenetic tree"
...
<!-- for some reason on the server get_label_from_node needs to be sourced directly, was not pb on laptop -->
...
<!-- I had to modify this one also now - 
to have the labels also shown in the plots which is
...
export_meta_treenodes_funV2(
  ...
  file = here::here(params$save_dir, "H5N1_HA_metadata.xlsx"),
  sheet = "H5N1_HA_collapsed_tree_metadata",
  append = FALSE,
...
circular_plot <- 
  ggtree(rooted_tree_nodelab, layout = "circular", color = "grey", alpha = 1, size = 1)  %<+%
  circular_meta +
  ggnewscale::new_scale_color() +
  geom_tippoint(
...
_Figure caption:_
H5N1 HA segment. ML phylogenetic tree. The light green dots represent the branching 
...
export_meta_treenodes_funV2(
  ...
  col_rep_tips = "tip_metalink",
  file = here::here(params$save_dir, "H5N1_HA_metadata.xlsx"),
  sheet = "H5N1_HA_circular_tree_metadata",
  append = TRUE,
  Isolate_Name, year, continent, country, Host_family, Host_latin
)
...
Tried to add directly in the tree but all jumbled so add in a panel --> 
```{r}
N529_plot <- 
  plot_subtree_N529 + 
...
export_meta_treenodes_funV2(
  ...
  col_rep_tips = "tip_metalink",
  file = here::here(params$save_dir, "H5N1_HA_metadata.xlsx"),
  sheet = "H5N1_HA_N529_tree_metadata",
  append = TRUE,
  Isolate_Name, year, continent, country, Host_family, Host_latin
)
...
export_meta_treenodes_funV2(
  ...
  col_rep_tips = "tip_metalink",
  file = here::here(params$save_dir, "H5N1_HA_metadata.xlsx"),
  sheet = "H5N1_HA_N9_tree_metadata",
  append = TRUE,
  Isolate_Name, year, continent, country, Host_family, Host_latin
)
```
...
xlsx::write.xlsx(as.data.frame(data_here) %>%
                   dplyr::select(-label), 
                 here::here(params$save_dir, "H5N1_HA_metadata.xlsx"),
