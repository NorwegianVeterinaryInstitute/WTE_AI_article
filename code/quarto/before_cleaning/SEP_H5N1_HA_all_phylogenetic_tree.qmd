...
explor_plot3b <- 
  ...
  ggplot2::geom_point(aes(x, y, color = category_highlight),
                      data = . %>% filter(isTip), 
                      position = ggforce::position_auto(), size = .8) +
  ggplot2::scale_color_manual(values = category_highlight_pal,
                              na.translate = FALSE,
...
# HERE adding solution tips labels
...
explor_plot3b$data <- 
  ...
  # Adding labels for nodes that became tips during collapsing
    mutate(Isolate_Id = 
             case_when(
...
# We do not do that - but is in metadata
# Host_latin_facet_df <- 
...
```{r}
ggsave(
  here::here(params$save_dir, "collapsed_plot_labels.svg"),
  collapsed_plot,
  device = "svg",
  dpi = 600, 
...
# HERE 
- I have to modify this one also now - to have the labels also shown in the plots which is
- changed name of sheets 


```{r}
#debug(export_meta_treenodes_funV2)
export_meta_treenodes_funV2(
  plot_data = explor_plot4$data,
  metadata = all_tree_metadata2,
  col_rep_tips = "tip_metalink",
  file = here::here(params$save_dir, "H5N1_metadata.xlsx"),
  sheet = "H5N1_collapsed_tree_metadata",
  Isolate_Name, year, continent, country, Host_family, Host_Latin_name
)

# Note cath has to fill the isolates names for the isolates she submitted 
# Said - now we are good for another round of metadata again ...
...
xlsx::write.xlsx(as.data.frame(data_here) %>%
                   dplyr::select(-label), 
                 here::here(params$save_dir, "H5N1_metadata.xlsx"),
