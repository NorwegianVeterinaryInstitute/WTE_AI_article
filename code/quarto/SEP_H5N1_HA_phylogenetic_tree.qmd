---
title: "SEP H5N1 HA Phylogenetic tree"
...
params:
    ...
    env_file: "results/env_files/H5N1_HA_SEP.RData"
...
tree_meta_incl_duplicates <- 
  ...
  dplyr::left_join(all_tree_metadata %>%  select(-subtype), c("represented_seq_label" = "label")) 
...
```{r}
tree_meta_incl_duplicates %>% 
  select("represented_seq_label") %>%
  rename("label" = "represented_seq_label") %>%
...
- I need the node labels for review 
```{r}
plot_adj <- 
  explor_plot3b + 
   ggplot2::geom_text(mapping = aes(x = branch, 
...
```{r}
manual_nodes_selection <- c("N1385", "N1251", "N402", "N1142")
```
...
_Figure Caption:_
H5N1 HA segment. Detail of the second group of isolates from the ML phylogenetic tree. 
...
```{r}
ggsave(
  here::here(params$save_dir, "Node_348_detail.svg"),
  N348_plotb,
  device = "svg",
  dpi = 600, 
  units = "cm", width = 35, height = 50
...
write_tsv(duplicates_df,
          file = here::here(params$save_dir, "duplicates.tsv")
          )
```

Metadata for all H5N1 isolates included
```{r}
data_here1 <- duplicates_df %>%
  select(represented_seq_label) %>%
  rename(label = represented_seq_label) %>%
...
data_here <- 
  dplyr::bind_rows(data_here1, data_here2) %>%
  distinct()

saveRDS(data_here, here::here(params$save_dir, "H5N1_HA_metadata.rds"))

xlsx::write.xlsx(as.data.frame(data_here), 
                 here::here(params$save_dir, "H5N1_HA_metadata.xlsx"),
                 sheetName = "H5N1_HA_metadata",
                 showNA = FALSE,
                 row.names = FALSE)
```
...
Note that one group of isolate isolates belonging to the same year of sampling
...
# Notes 
Data control is ok - its only H5N1 
<!-- I only had done my join in the wrong way --> 

```{r}
all_tree_metadata2 %>%
 select(label, Isolate_Id, subtype, continent, country, year,
         Host_family, Host_latin, Locality, lat, lon,  Isolate_Name) %>%
  mutate(GISAID_ID = if_else(str_detect(label, "EPI_ISL_"), label, NA_character_)) %>%
  filter(country == "Norway") %>%
  write_tsv(here::here(params$save_dir, "Norway_metadata_H5N1_H5N5.tsv"))
```
